...
if(_id == -1 && {GVAR(defaultSlotCost) != 0}) then {
	[
		[format ["This rifleman slot costs %1 tickets", GVAR(defaultSlotCost)], 5, 10, 10]
	] spawn BIS_fnc_EXP_camp_SITREP;
} else {
	[
		[format ["This %1 slot costs %1 tickets", toLower (GVAR(slotCost) select _id select 2), GVAR(slotCost) select _id select 1], 5, 10, 10]
	] spawn BIS_fnc_EXP_camp_SITREP;
};
